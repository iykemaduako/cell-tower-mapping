...
def load_trained_model(
    ...
    with open(
        Path(input_folder) / f"cell_classifier_labels_{trained_model}.pkl", "rb"
    ) as f:
    ...
    model.load_state_dict(
        torch.load(Path(input_folder) / f"cell_classifier_model_{trained_model}.pt")
    )
    return model
...
def get_paths_probs_of_cell_model_preds(
    input_folder: str = "datasets/all",
    transformation: transforms = None,
    batch_size: int = 30,
    shuffle: bool = False,
